...
export default function SurveyStep6Page() {
  const router = useRouter();

  const questions3 = useMemo(
    () => [
      "هل كان هناك توازن بين التعليم النظري و التطبيق العملي",
      "هل كان التدريب يحتوي على فرص تطبيق عملي كافية؟",
    ],
    []
  );

  const question2 = "هل كانت الأهداف التدريبية واضحة منذ البداية";

  const [answers3, setAnswers3] = useState(() =>
    Object.fromEntries(questions3.map((_, i) => [i, ""]))
  );
  const [answer2, setAnswer2] = useState("");

  const canNext =
    questions3.every((_, i) => answers3[i]) && Boolean(answer2);
  ...
  return (
    <div className="w-full">
      ...
      {/* ================= Steps Bar (الأخضر للنقطة الثالثة) ================= */}
      <div className="max-w-5xl mx-auto mb-10">
        <div className="relative">
          <div className="h-2 w-full rounded-full bg-gray-200" />
          <div className="absolute top-0 right-0 h-2 rounded-full bg-green-500 w-3/4" />
      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          <div className="space-y-14">
            {/* سؤال 1 + 2 (نعم/لا/جزئياً) */}
            {questions3.map((q, idx) => (
                  ...
                  {OPTIONS3.map((opt) => (
                    ...
                    >
                      ...
                      <input
                        ...
                        name={`q3-${idx}`}
                        value={opt.value}
                        checked={answers3[idx] === opt.value}
                        onChange={() =>
                          setAnswers3((prev) => ({ ...prev, [idx]: opt.value }))
                        }
                ...
                {OPTIONS2.map((opt) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      name="q2"
                      value={opt.value}
                      checked={answer2 === opt.value}
                      onChange={() => setAnswer2(opt.value)}
                      className="w-5 h-5 accent-green-600"
                    />
                  </label>
                ))}
        ...
        <div className="mt-10 flex items-center justify-center gap-6">
          <button
            onClick={() => router.push("/surveys/start/step-5")}
          ...
          <button
            disabled={!canNext}
            onClick={() => router.push("/surveys/start/step-7")}
